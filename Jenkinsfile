...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 4406
        NAMESPACE = "itincproject-site"
        REGISTRY_HOSTNAME = "itincproject"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "inctagram-project"
        DEPLOYMENT_NAME = "inctagram-project-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'itincproject-site') {
